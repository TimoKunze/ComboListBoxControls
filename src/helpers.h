...
/// \brief <em>Used to retrieve or set an item's \c lParam data as it is stored by \c ComboBoxEx32</em>
///
/// For \c DriveComboBox, we use the \c lParam member of the \c COMBOBOXEXITEM structure to store the
/// item's drive letter. The user-defined \c ItemData value is stored in an internal hash map. To make the
/// whole thing 100% transparent, we intercept \c CBEM_GETITEM and \c CBEM_SETITEM messages and return
/// (modify) the value out of (in) the hash map if \c CBEIF_PARAM is set. However, this prevents us
/// from retrieving (setting) an item's drive letter, so we introduce a custom \c CBEIF_* flag. If this
/// one is set, the \c DriveComboBox::OnGetItem (\c DriveComboBox::OnSetItem) method will remove it and
/// forward the message right to \c ComboBoxEx32.
///
/// \sa DriveComboBoxItem::get_ItemData, DriveComboBox::OnGetItem, DriveComboBox::OnSetItem
#define CBEIF_NOINTERCEPTION 0x80000000
...
/// \brief <em>Retrieves whether a combo box item index is a valid one</em>
///
/// \param[in] itemIndex The item index to check.
/// \param[in] pCBox The \c ComboBox object containing the item.
///
/// \return \c TRUE if the item index is valid; otherwise \c FALSE.
///
/// \sa IsValidListBoxItemIndex
BOOL IsValidComboBoxItemIndex(int itemIndex, ComboBox* pCBox);
/// \brief <em>Retrieves whether a combo box item index is a valid one</em>
///
/// \param[in] itemIndex The item index to check.
/// \param[in] pICBox The \c ImageComboBox object containing the item.
///
/// \return \c TRUE if the item index is valid; otherwise \c FALSE.
///
/// \sa IsValidListBoxItemIndex
BOOL IsValidComboBoxItemIndex(int itemIndex, ImageComboBox* pICBox);
/// \brief <em>Retrieves whether a combo box item index is a valid one</em>
///
/// \param[in] itemIndex The item index to check.
/// \param[in] pDCBox The \c DriveComboBox object containing the item.
///
/// \return \c TRUE if the item index is valid; otherwise \c FALSE.
///
/// \sa IsValidListBoxItemIndex
BOOL IsValidComboBoxItemIndex(int itemIndex, DriveComboBox* pDCBox);
/// \brief <em>Retrieves whether a combo box item index is a valid one</em>
///
/// \overload
///
/// \param[in] itemIndex The item index to check.
/// \param[in] hWndCBox The combo box control containing the item.
///
/// \return \c TRUE if the item index is valid; otherwise \c FALSE.
///
/// \sa IsValidListBoxItemIndex
BOOL IsValidComboBoxItemIndex(int itemIndex, HWND hWndCBox);
/// \brief <em>Retrieves whether a list box item index is a valid one</em>
///
/// \param[in] itemIndex The item index to check.
/// \param[in] pLBox The \c ListBox object containing the item.
///
/// \return \c TRUE if the item index is valid; otherwise \c FALSE.
///
/// \sa IsValidComboBoxItemIndex
BOOL IsValidListBoxItemIndex(int itemIndex, ListBox* pLBox);
/// \brief <em>Retrieves whether a list box item index is a valid one</em>
///
/// \overload
///
/// \param[in] itemIndex The item index to check.
/// \param[in] hWndLBox The list box control containing the item.
///
/// \return \c TRUE if the item index is valid; otherwise \c FALSE.
///
/// \sa IsValidComboBoxItemIndex
BOOL IsValidListBoxItemIndex(int itemIndex, HWND hWndLBox);
...
/// \brief <em>Retrieves the current ID of the \c commctrl_DragListMsg message</em>
///
/// \return The message's current ID.
///
/// \sa <a href="https://msdn.microsoft.com/en-us/library/bb761711.aspx">About Drag List Boxes</a>
UINT GetDragListMessage(void);
/// \brief <em>Retrieves the current ID of the \c DI_GETDRAGIMAGE message</em>
///
/// \return The message's current ID.
///
/// \sa ListBox::OnGetDragImage, ListBox::OLEDrag,
...
/// \return An \c HRESULT error code.
///
/// \sa DriveComboBox::Raise_OLEDragEnter,
...
/// \brief <em>Identifier of the window property used to store the instance that installed the mouse hook</em>
///
/// \sa MouseHookProc
#define HOOKPROPIDENTIFIER TEXT("{B9DD8FFA-5EF2-44d5-8EC3-E3A1886B693D}")
...
///
/// \return The value returned by \c IMouseHookHandler::HandleMessage.
///
/// \sa ImageComboBox::OnRButtonDown, DriveComboBox::OnRButtonDown, IMouseHookHandler::HandleMessage,
