...
FindDataEntry:
	...
	if(!pDataEntry->pData) {
		...
		if(properties.pOwnerDCBox) {
			//TODO: properties.pOwnerDCBox->Raise_OLESetData(static_cast<IOLEDataObject*>(this), formatID, pFormat->lindex, pFormat->dwAspect);
		} else if(properties.pOwnerICBox) {
			properties.pOwnerICBox->Raise_OLESetData(static_cast<IOLEDataObject*>(this), formatID, pFormat->lindex, pFormat->dwAspect);
		} else if(properties.pOwnerLBox) {
			properties.pOwnerLBox->Raise_OLESetData(static_cast<IOLEDataObject*>(this), formatID, pFormat->lindex, pFormat->dwAspect);
		}
	...
	if(properties.numberOfItemsToDisplay > 1) {
		...
		if(pEntry) {
			...
			*(pDataEntry->pFormat) = *pFormat;
			...
			if(properties.pOwnerDCBox) {
				properties.pOwnerDCBox->Raise_OLEReceivedNewData(static_cast<IOLEDataObject*>(this), formatID, pFormat->lindex, pFormat->dwAspect);
			} else if(properties.pOwnerICBox) {
				properties.pOwnerICBox->Raise_OLEReceivedNewData(static_cast<IOLEDataObject*>(this), formatID, pFormat->lindex, pFormat->dwAspect);
			} else if(properties.pOwnerLBox) {
				properties.pOwnerLBox->Raise_OLEReceivedNewData(static_cast<IOLEDataObject*>(this), formatID, pFormat->lindex, pFormat->dwAspect);
			}
...
void SourceOLEDataObject::SetOwner(DriveComboBox* pOwner)
{
	if(properties.pOwnerDCBox) {
...
void SourceOLEDataObject::SetOwner(ImageComboBox* pOwner)
{
...
void SourceOLEDataObject::SetOwner(ListBox* pOwner)
{
	...
	switch(format.cfFormat) {
		...
		if(pEntry) {
			...
			*(pDataEntry->pFormat) = format;
			...
			if(properties.pOwnerDCBox) {
				properties.pOwnerDCBox->Raise_OLEReceivedNewData(static_cast<IOLEDataObject*>(this), formatID, format.lindex, format.dwAspect);
			} else if(properties.pOwnerICBox) {
				properties.pOwnerICBox->Raise_OLEReceivedNewData(static_cast<IOLEDataObject*>(this), formatID, format.lindex, format.dwAspect);
			} else if(properties.pOwnerLBox) {
				properties.pOwnerLBox->Raise_OLEReceivedNewData(static_cast<IOLEDataObject*>(this), formatID, format.lindex, format.dwAspect);
			}
